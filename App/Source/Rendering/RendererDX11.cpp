...
#include "DDSTextureLoader.h"load
#include <algorithm>
...
ResourcePtr RendererDX11::LoadTexture( std::wstring filename /*, D3DX11_IMAGE_LOAD_INFO* pLoadInfo*/ )
{
	...
	if (extension == L"dds")
		hr = DirectX::CreateDDSTextureFromFile(m_pDevice.Get(), filename.c_str(), pResource.GetAddressOf(), nullptr);
	else
		hr = DirectX::CreateWICTextureFromFile(m_pDevice.Get(), pImmPipeline->m_pContext.Get(), filename.c_str(), pResource.GetAddressOf(), 0 );
